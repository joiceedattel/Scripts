...
[ $(ssh -T -o PasswordAuthentication=yes 10.5.250.139 mount | egrep "/efeap/data|/usr3" | grep -v "/Nfs-Shared/" |wc -l) != 2 ] && echo " Partitions are not mounted on DBSR..." >> /Rsync/rsync_out/rsync_out_"$dc_name"/rsync_"$DATE".out 2>> /Rsync/rsync_out/rsync_out_"$dc_name"/rsync_"$DATE".err  && exit 1
...
[ $(ps aux | grep "$dc_code" | grep -v $(echo $$) | grep -v "grep" |wc -l) != 0 ] && echo "Previous Rsync is running..." >> /Rsync/rsync_out/rsync_out_"$dc_name"/rsync_$DATE.out 2>> /Rsync/rsync_out/rsync_out_"$dc_name"/rsync_$DATE.err && exit 1
